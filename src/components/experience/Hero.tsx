...
export const Hero: React.FC<HeroProps> = ({ onEnter }) => {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // progress 0 → 1 as user scrolls through the first viewport height
      const progress = Math.min(Math.max(scrollY / vh, 0), 1);

      // Hero gently fades out as builder slides over it
      if (heroRef.current) {
        const heroOpacity = 1 - progress * 0.8;
        heroRef.current.style.opacity = String(Math.max(heroOpacity, 0));
      }
    };
  ...
  return (
    <>
      {/* Fixed hero — stays in place, builder slides over it */}
      <section
        ref={heroRef}
        className="fixed top-0 left-0 w-full h-[100svh] overflow-hidden bg-[#050807] z-0 will-change-[opacity]"
      >
      ...
      {/* Spacer — reserves space in the document flow so builder starts below hero */}
