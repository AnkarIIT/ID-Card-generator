...
export const GraphicPreview: React.FC<GraphicPreviewProps> = ({
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      {config.format === 'badge' ? (
        <div className="flex flex-col items-center gap-3 w-full">
          ...
          <div
            ...
            className="relative w-full aspect-[12/18.6] max-w-[420px] cursor-pointer group select-none"
            style={{ perspective: '1200px' }}
          >
            <div
              className="relative w-full h-full duration-700 transition-transform rounded-2xl shadow-2xl"
              style={{
            ...
            >
              {/* FRONT SIDE */}
              <div
                className="absolute inset-0 w-full h-full rounded-2xl overflow-hidden bg-[#071712] border border-[#d4af37]/40 shadow-2xl"
        ...
        <div className="relative w-full aspect-square max-w-[440px] rounded-2xl bg-[#071712] border border-[#d4af37]/40 shadow-2xl overflow-hidden p-2">
          <canvas ref={frameCanvasRef} className="w-full h-full object-contain rounded-xl" />

          {!userImage && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#071712]/80 backdrop-blur-xs p-6 text-center">
              <div className="rounded-full bg-[#f3c85c]/20 p-3 text-[#f3c85c] mb-2 border border-[#f3c85c]/30">
                <Sparkles className="h-6 w-6" />
              </div>
              <p className="font-bold text-[#f7eec8] text-base">Photo Recommended</p>
              <p className="text-[#a2b8ad] text-xs mt-1 max-w-xs">
                Upload your photo on the left panel to preview your HH Goa 2026 graphic in real-time!
              </p>
            </div>
          )}
        </div>
      ...
      {/* Quick Side Download Switcher for Badge Format */}
      {config.format === 'badge' && (
        <div className="flex items-center gap-2 w-full max-w-[420px]">
          <button
            type="button"
            onClick={() => handleDownload('front')}
            className="flex-1 py-1.5 px-2 rounded-lg bg-[#0a231b] border border-[#d4af37]/30 text-[#f7eec8] text-xs font-semibold hover:border-[#f3c85c] hover:bg-[#103025] transition-all cursor-pointer flex items-center justify-center gap-1"
          >
      ...
      <div className="grid grid-cols-2 gap-2.5 w-full max-w-[420px]">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#091f17] border border-[#d4af37]/30 hover:border-[#f3c85c] px-3.5 py-2.5 font-semibold text-[#f7eec8] text-xs sm:text-sm transition-all cursor-pointer disabled:opacity-50"
        >
          {isSharing === 'x' ? (
            <Loader2 className="h-4 w-4 animate-spin text-[#f3c85c]" />
          ) : (
            <Twitter className="h-4 w-4 fill-current text-[#f3c85c]" />
          )}
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#f3c85c] hover:bg-[#fbd881] text-[#071712] px-3.5 py-2.5 font-bold text-xs sm:text-sm shadow-md transition-all cursor-pointer active:scale-95"
        >
      ...
      {showShareModal && shareResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="relative w-full max-w-md rounded-2xl border border-[#d4af37]/40 bg-[#071712] p-6 shadow-2xl space-y-4">
            <button
              onClick={() => setShowShareModal(false)}
              className="absolute top-4 right-4 text-[#a2b8ad] hover:text-white cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="flex items-center gap-2 text-[#f3c85c] font-bold text-lg">
              ...
              <span>Graphic Ready to Share!</span>
            </div>

            <p className="text-[#a2b8ad] text-xs leading-relaxed">
              Your graphic is generated and hosted with Open Graph meta tags. When posted, rich card preview images will render automatically!
            </p>

            {/* Generated Image Preview inside Modal */}
            <div className="rounded-xl overflow-hidden border border-[#d4af37]/30 max-h-48 flex justify-center bg-[#030d09] p-2">
              <img src={shareResult.ogImageUrl} alt="Generated HH Goa Graphic" className="object-contain h-full rounded-lg" />
            </div>

            {/* Share URL Input Box */}
            <div className="space-y-1">
              <label className="text-[11px] text-[#a2b8ad] uppercase font-semibold">Your Live Card Link</label>
              <div className="flex items-center gap-2 rounded-lg bg-[#030d09] p-1.5 border border-[#d4af37]/30">
                <input
                  type="text"
                  readOnly
                  value={shareResult.shareUrl}
                  className="w-full bg-transparent text-xs text-[#f3c85c] px-2 focus:outline-none"
                />
                <button
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-500 py-2.5 font-bold text-white text-xs hover:opacity-90"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-blue-800 py-2.5 font-bold text-white text-xs hover:opacity-90"
              >
